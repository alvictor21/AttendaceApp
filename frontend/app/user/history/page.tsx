...
// ── Types ──────────────────────────────────────────────────────────────────────
type StatusBackend = "On Time" | "Late" | "Sakit" | "Izin" | "Alpha";
...
export default function AttendanceHistoryPage() {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      try {
        ...
        const response = await fetch("http://127.0.0.1:8000/api/absensi/history", {
          headers: {
  ...
  if (loading) {
    return <div className="p-5 text-gray-400">Memuat riwayat absensi...</div>;
  }
    ...
    <div className="bg-gray-100 min-h-screen pb-24 font-sans">
      ...
      <div className="bg-white px-5 pt-10 pb-5 border-b border-gray-100">
        <div className="flex items-center justify-between">
          ...
          <button className="flex items-center gap-1.5 border border-gray-200 rounded-lg px-3 py-1.5 text-[12px] font-semibold text-gray-600 bg-white shadow-sm active:bg-gray-50">
